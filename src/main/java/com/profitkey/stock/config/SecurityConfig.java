...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers(RequestMatcherPaths.PERMIT_ALL_PATHS).permitAll()  // permitAllPaths는 누구나 접근 가능
                        .requestMatchers(RequestMatcherPaths.AUTHENTICATED_PATHS).authenticated()  // authenticatedPaths는 인증된 사용자만
    ...
    public UrlBasedCorsConfigurationSource corsConfigurationSource() {
        ...
        configuration.setAllowedOrigins(List.of("http://localhost:3000"));  // React에서 보내는 요청을 허용
